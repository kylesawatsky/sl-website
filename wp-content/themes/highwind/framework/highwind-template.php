...
if ( ! function_exists( 'highwind_sidebar' ) ) {
	function highwind_sidebar() {
		if ( ! is_page_template( 'templates/template-fullwidth.php' ) && !is_search() ) {
			if ( is_page() ) {
				get_sidebar( 'page' );
			} else if ( is_single() ) {
				get_sidebar( 'post' );
...
if ( ! function_exists( 'highwind_content_nav' ) ) {
	function highwind_content_nav( $nav_id ) {
		...
		?>
		<nav role="navigation" id="<?php echo $nav_id!="" ? esc_attr( $nav_id ) : "empty-navigation"; ?>" class="<?php echo $nav_class; ?>">
...
function highwind_back_to_top() {
	?>
		<a href="#top" class="back-top-button">
			<img class="back-top-arrow" src="/wp-content/uploads/2013/12/arrow-grey.png" alt="" />
		</a>
	<?php
}