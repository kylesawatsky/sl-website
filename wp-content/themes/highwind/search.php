...
<section class="content" role="main">
	...
	<?php if ( have_posts() ) : ?>
		...
		<?php
			...
			switch($posttype) {
				case 'sl_content_block':
					$id = get_the_ID();
					$title = get_the_title();
					$meta = get_post_meta($id, "metaattached", true);
					$attachedPage = $meta[0]["attached"];
					$anchor = $meta[0]["anchor-name"];
?>
					<h3><a href="<?php echo get_site_url() . "/" . $attachedPage . "#block-" . $anchor; ?>"><?php echo $title ?></a></h3>
					<?php the_excerpt(); break; ?>
			<?php	default: ?>
